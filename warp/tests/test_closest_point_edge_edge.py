...
def _well_conditioned_mask(p1, q1, p2, q2):
    ...
    in_D = ((t_init > margin) & (t_init < 1.0 - margin) &
            (s_init_uc > margin) & (s_init_uc < 1.0 - margin))

    # CASE_E: t firmly clamped to 0; s_uc = -c/a must be interior
    s_E_uc = -c / safe_a
    in_E = ((t_init < -margin) &
            (s_E_uc > margin) & (s_E_uc < 1.0 - margin))

    # CASE_F: t firmly clamped to 1; s_uc = (b-c)/a must be interior
    s_F_uc = (b - c) / safe_a
    in_F = ((t_init > 1.0 + margin) &
            (s_F_uc > margin) & (s_F_uc < 1.0 - margin))
...
def test_edge_edge_grad_finite_difference(test, device):
    ...
    for name, g, fd in [("p1", g_p1, fd_p1), ("q1", g_q1, fd_q1),
                        ("p2", g_p2, fd_p2), ("q2", g_q2, fd_q2)]:
...
def test_edge_edge_grad_bounded_near_parallel(test, device):
    ...
    for name, g in [("p1", g_p1), ("q1", g_q1), ("p2", g_p2), ("q2", g_q2)]:
        ...
        test.assertLess(max_abs, bound,
                        msg=f"{name} gradient magnitude {max_abs:.3g} exceeds bound {bound}")
