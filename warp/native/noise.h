...
)
{
    ...
    for (uint32 i = 0; i < octaves; ++i) {
        ...
        adj_xy[0] += k * (-H.data[0][1] * adj_ret[0] + H.data[0][0] * adj_ret[1]);
...
)
{
    ...
    for (uint32 i = 0; i < octaves; ++i) {
        ...
        float k = amplitude * freq;
        for (int c = 0; c < 3; ++c) {
            adj_xyz[c] += k * (adj_ret[0] * (H3.data[1][c] - H2.data[2][c])
                             + adj_ret[1] * (H1.data[2][c] - H3.data[0][c])
...
)
{
    ...
    for (uint32 i = 0; i < octaves; ++i) {
        ...
        float k = amplitude * freq;
        for (int c = 0; c < 4; ++c) {
            adj_xyzt[c] += k * (adj_ret[0] * (H3.data[1][c] - H2.data[2][c])
                              + adj_ret[1] * (H1.data[2][c] - H3.data[0][c])
                              + adj_ret[2] * (H2.data[0][c] - H1.data[1][c]));
        }
