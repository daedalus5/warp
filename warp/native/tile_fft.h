...
inline CUDA_CALLABLE void cooperative_fft_radix2(vec_t<2, T>* x, int n, int log_n, int direction_sign)
{
    ...
    for (int len = 2; len <= n; len <<= 1) {
        const int half = len >> 1;
        const T theta_base = T(direction_sign) * fft_cpu_detail::fft_consts<T>::two_pi() / T(len);
...
inline CUDA_CALLABLE void tile_fft_gpu_impl(int batch, int ept, int shared_bytes, Tile& Xinout)
{
    ...
    const int log_n = fft_cpu_detail::fft_log2_pow2(fft_size);
    ...
    for (int b = 0; b < batch; ++b) {
        ...
        fft_gpu_detail::cooperative_fft_radix2<T>(
            reinterpret_cast<vec_t<2, T>*>(scratch), fft_size, log_n, DirectionSign
        );
