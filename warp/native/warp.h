...
// layer (the cluster shape itself is set via __cluster_dims__ in codegen).
...
// Query the maximum cluster size (CTA count, 1D product) usable for *kernel*
...
WP_API int wp_cuda_get_max_cluster_size(void* context, void* kernel, int block_dim, int dynamic_smem_bytes);
