...
//
// The general nondegenerate case factors through
//     denom    = |d1|² |d2|² - (d1·d2)²    where d1 = q1 - p1, d2 = q2 - p2
//     num      = (d1·d2)(d2·r) - (d1·r)|d2|²    where r = p1 - p2
// and computes s_init = clamp(num / denom, 0, 1). When the edges are
// near-parallel both quantities suffer catastrophic cancellation:
// algebraically denom = |d1|²|d2|² sin²θ but the formula a·e - b² subtracts
// two equal large numbers (the "1 - cos²θ" antipattern). Float32 produces
// pure noise once sinθ approaches sqrt(machine ε).
//
// We extract sin² directly via Lagrange's identities:
//     |cross(d1, d2)|² = |d1|²|d2|² - (d1·d2)²        = denom
//     cross(d1, d2) · cross(d2, r) = (d1·d2)(d2·r) - (d1·r)|d2|²  = num
// preserving precision down to angles where cross(d1, d2) itself is at the
// roundoff floor. The "non-degenerate" test is a relative-tolerance check
// against |d1|²|d2|² so we don't divide by quantities at machine epsilon
// (which would amplify upstream rounding through the gradient).
/*
...
// Relative tolerance below which the edges are treated as parallel.
// denom / (a*e) = sin²θ; this threshold corresponds to sinθ ≈ 1e-3 (~0.06°),
// well below the float32 noise floor of (a·e − b²).
static constexpr float kClosestPointEdgeEdgeParallelTol = 1.0e-6f;

static CUDA_CALLABLE vec3
closest_point_edge_edge(vec3 p1, vec3 q1, vec3 p2, vec3 q2, float32 epsilon)
{
    const vec3 d1 = q1 - p1;
    const vec3 d2 = q2 - p2;
    const vec3 r = p1 - p2;
    ...
    if (degen1 && degen2) {
        ...
        // edge 1 is a point: project p1 onto edge 2
        t = f / e;
    } else if (degen2) {
        // edge 2 is a point: project p2 onto edge 1
        ...
        const float denom = dot(n, n);  // = a*e - b² via Lagrange, robustly

        float s_init;
        if (denom > kClosestPointEdgeEdgeParallelTol * a * e) {
            // num = b*f - c*e via Lagrange (cross(d1,d2) · cross(d2,r))
            const float num = dot(n, cross(d2, r));
            s_init = clamp(num / denom, 0.0f, 1.0f);
        } else {
            // edges parallel within tolerance; pick s = 0 and let t correct
            s_init = 0.0f;
        }
...
//   B: edge 1 degenerate (s = 0, t = f/e)
//   C: edge 2 degenerate (t = 0, s = clamp(-c/a, 0, 1))
//   D: general interior (s, t in (0, 1) after t-correction)
//   D_DEGEN: edges parallel within tolerance (s = 0, t = f/e)
...
// applies the chain rule. The general case D uses the cross-product form of
// denom and num so the gradient is well-conditioned at small angles.
...
)
{
    ...
    float denom = 0.0f;   // |cross(d1, d2)|² in case D
    float s_uc = 0.0f;    // unclamped s_init for the active branch
    vec3 n(0.0f);         // cross(d1, d2) for case D
    vec3 m(0.0f);         // cross(d2, r)  for case D
    ...
    if (degen1 && degen2) {
        ...
        n = cross(d1, d2);
        denom = dot(n, n);
        const bool denom_ok = (denom > kClosestPointEdgeEdgeParallelTol * a * e);
        ...
        if (t_init < 0.0f) {
            ...
            branch = denom_ok ? CASE_D : CASE_D_DEGEN;
            s = s_init;
            t = t_init;
        }
    }
    ...
    case CASE_D: {
        ...
        // s = clamp(num/denom, 0, 1) where num = dot(n, m), denom = dot(n, n),
        // n = cross(d1, d2), m = cross(d2, r)
        float adj_s_uc = 0.0f, _u_lo = 0.0f, _u_hi = 0.0f;
        adj_clamp(s_uc, 0.0f, 1.0f, adj_s_uc, _u_lo, _u_hi, adj_s);
        ...
        // adj of dot(n, m) and dot(n, n) into adj_n, adj_m
        const vec3 adj_n = adj_num * m + (2.0f * adj_denom) * n;
        const vec3 adj_m = adj_num * n;

        // adj of cross(d1, d2) -> adj_d1, adj_d2
        adj_d1 += cross(d2, adj_n);
        adj_d2 -= cross(d1, adj_n);
        // adj of cross(d2, r) -> adj_d2, adj_r
        adj_d2 += cross(r, adj_m);
        adj_r  -= cross(d2, adj_m);
        break;
    }
    case CASE_D_DEGEN:
        // denom too small (parallel): s held at 0, t = f/e, adj_s discarded.
        adj_f += adj_t / e;
        adj_e += -adj_t * t / e;
        break;
    }
