...
def codegen_kernel(kernel, device, options):
    ...
    # (1, 1, 1) is the implicit default and is treated as a no-op so that
    # kernels without cluster_dim produce byte-identical source to pre-feature.
    cluster_dims_str = ""
    if device == "cuda":
        cluster_dim = options.get("cluster_dim", (1, 1, 1))
        if cluster_dim != (1, 1, 1):
            cluster_dims_str = f"WP_CLUSTER_DIMS({cluster_dim[0]}, {cluster_dim[1]}, {cluster_dim[2]}) "
