...
"""Whole-element overwrite baseline on ``wp.array(dtype=wp.vec3)``: the
cheapest ``array_store`` pattern. Reference for "cost of one element-sized
store" — any slot-write lowering that approaches this is not regressing
the element-write pattern the hardware already does efficiently.
"""
...
class RunBackwardKernel:
    def setup(self):
        ...
        self.src = wp.ones(self.n, dtype=wp.vec3, device="cuda:0", requires_grad=True)
        wp.synchronize_device("cuda:0")
