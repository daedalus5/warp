...
"""Augassign struct-field on a user ``@wp.struct`` array:
``dst[i].position += p[i]`` (legacy path until augassign lowering lands).
"""
...
class RunForwardKernel:
    def setup(self):
        ...
        self.cmd = wp.launch(
            state_augassign_field, self.n, inputs=[self.dst, self.p], device="cuda:0", record_cmd=True
        )
