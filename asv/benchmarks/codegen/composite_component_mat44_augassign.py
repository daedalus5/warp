...
"""Augassign scalar-element on ``wp.array(dtype=wp.mat44)``:
``dst[i][1, 2] += src[i]`` (legacy path until augassign lowering lands).
"""
...
class RunForwardKernel:
    def setup(self):
        ...
        self.cmd = wp.launch(
            mat44_augassign_elem, self.n, inputs=[self.dst, self.src], device="cuda:0", record_cmd=True
        )
