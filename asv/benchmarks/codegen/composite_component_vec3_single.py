...
"""Single-slot write on ``wp.array(dtype=wp.vec3)``: ``dst[i].y = src[i]``.

Tracks compile time (CUDA codegen + NVRTC), forward-only kernel runtime,
and forward+backward kernel runtime. One kernel per file keeps compile-time
benchmarks isolated: ``wp.load_module`` compiles every kernel in the module,
so co-habitating kernels inflate each other's numbers.
"""
